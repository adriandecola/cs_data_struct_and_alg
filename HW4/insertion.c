...
#define EXPERIMENT 1 // use this for the experimentation in this assignment
...
void sortIntegers(int* array, unsigned int size){
	unsigned int i;
	for (i = 1; i < size; i++){                //the first element is already sorted
		// moving the new element down until it is in the correct position
...
int main(int argc, char* argv[]){
  ...
  int i;
  for(i = 0 ; i < size ; i++) {
    // Generate random values from 0 to size
    random[i] = rand()%size; 
  }
